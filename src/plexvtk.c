...
/* //The original version, I think this is good for the tecplot format output.
#undef __FUNCT__
...
#define __FUNCT__ "DMPlexVTKWriteField_ASCII"
/*
Modified by Rongliang Chen on 12/4/2014
The original version does not surport the field by field view
which is surportted in this version.
*/
...
PetscErrorCode DMPlexVTKWriteField_ASCII(DM dm, PetscSection section, PetscSection globalSection, Vec field, const char name[], FILE *fp, PetscInt enforceDof, PetscInt precision, PetscReal scale)
{
  ...
  if (nfields == 0){
    ...
    for (i = 0; i < nfields; i++){
      ...
      ierr = PetscSectionGetFieldName(section, i, &fieldName);CHKERRQ(ierr);
      ierr = PetscSectionGetField(section, i, &subSection);CHKERRQ(ierr);

      ierr = PetscSectionGetChart(subSection, &pStart, &pEnd);CHKERRQ(ierr);
      for (p = pStart; p < pEnd; ++p) {
        ierr = PetscSectionGetDof(subSection, p, &numDof);CHKERRQ(ierr);
        if (numDof) break;
      }
      ...
      if (maxDof == 3) {
        ...
        ierr = PetscFPrintf(comm, fp, "SCALARS %s double %d\n", fieldName, maxDof);CHKERRQ(ierr);
        ierr = PetscFPrintf(comm, fp, "LOOKUP_TABLE default\n");CHKERRQ(ierr);
      }
      ierr = DMPlexVTKWriteSection_ASCII(dm, subSection, globalSection, field, fp, enforceDof, precision, scale);CHKERRQ(ierr);
  }
  }
