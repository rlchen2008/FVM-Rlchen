...
PetscErrorCode DMPlexCreateExodusFromFile(MPI_Comm comm, const char filename[], PetscBool interpolate, DM *dm)
{
  ...
  ierr = PetscPrintf(comm,"Reading mesh file: %s\n",filename);CHKERRQ(ierr);
#if defined(PETSC_HAVE_EXODUSII)
  if (!rank) {
...
PetscErrorCode DMPlexCreateExodus(MPI_Comm comm, PetscInt exoid, PetscBool interpolate, DM *dm)
{
  ...
  if (!rank) {
    ...
    for (cs = 0, c = 0; cs < num_cs; cs++) {
      ...
      num_cells += num_cell_in_set;
    }
    ierr = PetscPrintf(comm,"Number of blocks %d and number of elements %d in the mesh!\n",cs, num_cells);CHKERRQ(ierr);
  ...
  if (!rank) {
    ...
    if (dim > 0) {
      for (v = 0; v < numVertices; ++v) {
        if (PetscAbsScalar(x[v])<1.e-8) x[v] = 0; // ignore the very small number, this could be a problem if the mesh is very fine, such as the mesh size is smaller then 1.e-8. is this necessary? not sure!
        coords[v*dim+0] = x[v];
      }
    }
    if (dim > 1) {
      for (v = 0; v < numVertices; ++v) {
        if (PetscAbsScalar(y[v])<1.e-8) y[v] = 0; // ignore the very small number, this could be a problem if the mesh is very fine, such as the mesh size is smaller then 1.e-8. is this necessary? not sure!
        coords[v*dim+1] = y[v];
      }
    }
    if (dim > 2) {
      for (v = 0; v < numVertices; ++v) {
        if (PetscAbsScalar(z[v])<1.e-8) z[v] = 0; // ignore the very small number, this could be a problem if the mesh is very fine, such as the mesh size is smaller then 1.e-8. is this necessary? not sure!
        coords[v*dim+2] = z[v];
      }
    }
    ierr = PetscFree3(x,y,z);CHKERRQ(ierr);
  }
  /*The coord is ordered node by node, that is coord=[x1,y1,z1, x2,y2,z2, ......]*/
