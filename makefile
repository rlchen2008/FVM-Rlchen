...
runtest1:
	...
	-includeenergy -max_time_its 5000 -ts_view -ts_type rk -PressureFlux \
...
SNESOPTIONS = -snes_rtol 1.e-6 -snes_max_it 10
...
runimplicit:
	-@${MPIEXEC} -n 1 ./AeroSim -show_partition -f ./../meshes/3DCouetteFVM64.exo \
	${SNESOPTIONS} ${KSPOPTIONS} ${PCOPTIONS} \
	-time_integral_method 1 -initial_time 0.0 -final_time 10 -dt 0.01 -timestep 0 -max_time_its 1000 \
